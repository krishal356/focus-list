# AGENTS.md — Operational Instructions for FocusList Challenge

---

## 1. Mission

You are an autonomous senior frontend software engineering agent. Your objective is to build, verify, deploy, and submit **FocusList**—a high-performance, responsive, accessible, frontend-only task management web application—within a strict **1-hour challenge window**, maximizing the evaluation score under the **FAIE (Frontend Arena Interactive Evaluation) System**.

---

## 2. Project Context

- **Challenge**: FocusList Demo/Practice Challenge on Frontend Arena (`frontendarena.online`).
- **Goal**: Full implementation of all functional requirements, deterministic state handling, Local Storage persistence, responsive UI, WCAG AA accessibility, clean build, and verified live deployment.
- **Target Evaluation Factors**: Functional correctness (100%), UI/UX hierarchy, mobile/desktop responsiveness, accessibility (screen readers + keyboard), performance, and code quality.
...
6. **Mandatory Live Deployment**: A working live production URL must be deployed and verified against the Acceptance Test Plan before concluding the task.
7. **Strict 1-Hour Timebox**: Never waste time on decorative extras before all P0 functional and evaluation requirements are 100% complete and verified.
...
Antigravity must NOT use browser/Chrome/DOM inspection tools to visually review, validate, or judge the FocusList UI unless the human developer explicitly requests browser automation.
...
1. **Creation**: Create a High-priority task `"Verify FAIE Submission"`. Confirm it renders with badge.
...
## 27. FAIE-Oriented Final Audit Checklist

| Dimension | Verification Item | Status |
| :--- | :--- | :--- |
| **Functionality** | Create, Edit, Delete, Toggle, Priority, Search, Filter, Stats, Persistence | Required |
| **Data Safety** | Single source of truth, derived stats, safe `localStorage` parse | Required |
| **UI/UX** | Clear visual hierarchy, distinct badges, completed styling, empty states | Required |
...
| **Performance** | Instant interactions, zero lag, lightweight bundle | Required |
| **Code Quality** | Modular components, clean TypeScript interfaces, zero application-caused console errors | Required |
| **Deployment** | Publicly accessible live deployment URL verified | Required |
...
- [ ] All 12 P0 functional requirements are completely implemented and operating bug-free.
- [ ] Global task statistics reflect the full dataset regardless of active filters.
- [ ] Task persistence across page refresh is verified.
- [ ] Mobile and desktop layouts display without horizontal overflow.
- [ ] Full keyboard navigation and accessible `aria-label` tags are verified.
- [ ] Production build succeeds with 0 errors.
- [ ] Live deployment URL is live, reachable, and verified.
- [ ] All items in the FAIE Final Audit checklist pass.
...
FocusList is a conventional frontend-only productivity application.
...
All task creation, filtering, searching, statistics, editing, completion, deletion, and persistence must be implemented using ordinary frontend application logic.
...
- Does it reinforce FocusList's identity?
- Does it preserve performance and accessibility?
- Does it preserve the 1-hour challenge scope?
